...
async function Create_2StepVerification(user) { 
    const Already_hasStepVerification = await StepVerification.findOne({username : user.username})
    if (Already_hasStepVerification) await Already_hasStepVerification.delete()
...
router.post('/login', Should_Not_Be_Authenticated , Validate_Login, async (req, res, next) => { 
    try {
...
ValidateValueByChoice(['query', 'type'], ['email', 'pgp']), 
 Validate_EncryptedCodeQuery, async (req, res) => { 
    try {
        res.render('2fa')
    } catch (e) {
        console.log(e)
...
router.post('/2fa', Should_Not_Be_Authenticated, 
ValidateValueByChoice(['query', 'type'], ['email', 'pgp']), 
Validate_EncryptedCodeQuery, Validate_Code, async (req, res, next) => { 
    try {
