...
function testFilesMimetype(files, allowedMimetype) {
    for(let i = 0; i < files.length; i++) {
        ...
        if (!allowedMimetype.includes(filesMimetype)) throw new Error(`We only accept those kinds of files type: ${allowedMimetype.join(', ')}`)
    }
    return 
}

function ImageUploadsValidation({min = 0, max = 3, maxSize = 5, isRequired = false, errorUrl = '/404', allowedMimetype = ['png', 'jpg', 'jpeg']} = {}) {
    return (req, res, next) => {
        try {
            if (req.files) {
...
module.exports = {uploadsFiles, ImageUploadsValidation, deleteImage, renameImage, generateRandomName}