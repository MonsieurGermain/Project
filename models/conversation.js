...
function Create_New_Message(Message, Sender, sender_1, Settings, Message_Expiring_Settings) {
    ...
    if (Settings.timestamps === true) New_Message.timestamps = format(new Date(), "HH:mm LLLL dd yyyy") 

    if (Message_Expiring_Settings) New_Message.expire_at = Date.now() + Message_Expiring_Settings * 86400000  
